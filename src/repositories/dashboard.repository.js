...
export async function kpiStock() {
...
export async function sumMovimientos(desdeDT, hastaDT) {
  const [[row]] = await pool.query(
    `
    SELECT
      COALESCE(SUM(CASE WHEN tipo_movimiento IN (?) THEN cantidad ELSE 0 END), 0) AS entradas,
      COALESCE(SUM(CASE WHEN tipo_movimiento IN (?) THEN cantidad ELSE 0 END), 0) AS salidas,
      COUNT(*) AS movimientos,
      COALESCE(SUM(CASE WHEN tipo_movimiento IN ('Ajuste positivo','Ajuste negativo','Correccion') THEN cantidad ELSE 0 END), 0) AS ajustes_unidades,
      SUM(CASE WHEN tipo_movimiento IN ('Ajuste positivo','Ajuste negativo','Correccion') THEN 1 ELSE 0 END) AS ajustes_count
    FROM movimientos_inventario
    WHERE fecha_movimiento BETWEEN ? AND ?
  `,
    [TIPOS_ENTRADA, TIPOS_SALIDA, desdeDT, hastaDT]
  );
  return row;
}

export async function movimientosDiarios(desdeDT, hastaDT) {
...
export async function topProductosSalidas(desdeDT, hastaDT, limite = 10) {
...
export async function movimientosPorTipo(desdeDT, hastaDT) {
  const [rows] = await pool.query(
    `
    SELECT tipo_movimiento, COUNT(*) AS registros, COALESCE(SUM(cantidad), 0) AS unidades
    FROM movimientos_inventario
    WHERE fecha_movimiento BETWEEN ? AND ?
...
export async function alertasStock() {
  const [bajo] = await pool.query(`
    ...
    LEFT JOIN producto_imagenes i
      ON i.id_producto = p.id_producto AND i.es_principal = 1 AND i.activo = 1
    WHERE p.activo = 1 AND p.stock_actual > 0 AND p.stock_actual <= p.stock_minimo
...
export async function sinMovimiento(dias = 30) {
